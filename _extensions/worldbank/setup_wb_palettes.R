...
#'
#' Use \code{\link{wb_palette}} to construct palettes of desired length.
#'
#' @export
wb_palettes <- list("DIME" = c("Dark" = "#07202E",
                                 "MediumDark" = "#296586",
                                 "Medium" = "#4090BE",
                                 "MediumLight" = "#66BDE6",
                                 "Light" = "#B7DEF3"),
...
#' # colors
#' pal <- wb_palette(21, name = "DIME", type = "continuous")
#' image(volcano, col = pal)
wb_palette <- function(name, n, type = c("discrete", "continuous")) {
  type <- match.arg(type)
  pal <- wb_palettes[[name]]
  if (is.null(pal))
